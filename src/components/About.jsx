...
export default function About() {
  ...
  return (
    <section id="about" ref={ref} className="py-16 sm:py-20 md:py-28 relative overflow-hidden">
      <div className="absolute inset-0 -z-10">
        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-48 h-48 sm:w-64 sm:h-64 bg-brand-600/5 rounded-full blur-3xl" />
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        >
          <p className="font-mono text-brand-400 text-[10px] sm:text-xs tracking-widest uppercase mb-2 sm:mb-3">Who I Am</p>
          <h2 className="section-heading text-3xl sm:text-4xl md:text-5xl">About Me</h2>
        ...
        <div className="grid lg:grid-cols-2 gap-10 sm:gap-12 lg:gap-16 items-start">
          ...
          >
            ...
            <p className="text-slate-400 text-sm sm:text-base leading-relaxed mb-6 sm:mb-10">
          ...
          >
            {facts.map((fact, i) => (
              <motion.div
                ...
                className="glass-hover rounded-xl p-3 sm:p-4 flex items-center gap-3 sm:gap-4 group"
              >
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-brand-600/10 border border-brand-600/20 flex items-center justify-center text-brand-400 group-hover:bg-brand-600/20 transition-colors shrink-0">
                  <fact.icon size={14} className="sm:w-4 sm:h-4" />
                </div>
                <div className="min-w-0">
                  <p className="text-slate-500 text-[10px] sm:text-xs font-mono">{fact.label}</p>
                  <p className="text-slate-200 text-sm sm:text-base font-medium truncate">{fact.value}</p>
                </div>
              </motion.div>
            ))}
            ...
            <motion.div
              ...
              className="glass rounded-xl p-4 sm:p-5 mt-2"
            >
              <p className="text-slate-400 text-[10px] sm:text-xs font-mono uppercase tracking-widest mb-2 sm:mb-3">Core Strengths</p>
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {['Problem-Solving Mindset', 'Quick Learner', 'Team Player', 'Data-Driven'].map((s) => (
                  <span key={s} className="tag text-[10px] sm:text-xs">{s}</span>
                ))}
              </div>
            </motion.div>
          </motion.div>
